		...
		const char *pattern)
{
	...
	for (i = 0; i < LED_DESC_CNT; ++i) {
		...
		if (brightness > 0 && scaled < LED_BRIGHTNESS_MIN)
...
static void led_desc_enumerate(void)
{
	...
	while ((ent = readdir(dir)) != NULL) {
		...
		if (!strcmp(ent->d_name, ".."))
			continue;
		if (!strcmp(ent->d_name, "not-connected"))
			continue;

		if (!strcmp(ent->d_name, xstr(LED_RED_NAME)))
			desc->led_class = LED_RED;
		else if (!strcmp(ent->d_name , xstr(LED_GREEN_NAME)))
			desc->led_class = LED_GREEN;
		else if (!strcmp(ent->d_name, xstr(LED_BLUE_NAME)))
			desc->led_class = LED_BLUE;
		else if (use_bl_class)
			continue;
		else if (!strcmp(ent->d_name, xstr(LED_BACKLIGHT_NAME)))
			desc->led_class = LED_BACKLIGHT;
#ifdef LED_BACKLIGHT2_NAME
		else if (!strcmp(ent->d_name, xstr(LED_BACKLIGHT2_NAME)))
			desc->led_class = LED_BACKLIGHT;
#endif
#ifdef LOGO_BACKLIGHT_NAME
		else if (!strcmp(ent->d_name, xstr(LOGO_BACKLIGHT_NAME)))
			desc->led_class = LED_BACKLIGHT;
#endif
#ifdef LOGO_BACKLIGHT2_NAME
		else if (!strcmp(ent->d_name, xstr(LOGO_BACKLIGHT2_NAME)))
			desc->led_class = LED_BACKLIGHT;
#endif
		else
			continue;
...
struct hw_module_t HAL_MODULE_INFO_SYM = {
	...
	.author = "Bjorn Andersson <bjorn.andersson@sonymobile.com>, Giulio Cervera <giulio.cervera@cyanogenmod.org>, Chirayu Desai <cdesai@cyanogenmod.org>",
